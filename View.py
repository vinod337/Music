...
class View:
    def __init__(self, top=None):
        ...
        self.skip_scale = ttk.Scale(top)
        self.skip_scale.place(relx=0.393, rely=0.22, relwidth=0.495, relheight=0.0
                , height=7, bordermode='ignore')
        self.skip_scale.configure(takefocus="")
    ...
    def setup_player(self):
        ...
        self.skip_scale.config(from_=0,to=100,command=self.skip_song)
        #self.skip_scale.set(0)
    ...
    def show_timer(self,total_sec):
        curr_sec=1
        if self.isPaused==True or self.skipped==True:
            curr_sec=self.current_val
            self.isPaused=False
            self.skipped=False
            #print(f"Total {total_sec} and curr{curr_sec}")
        ...
        while curr_sec<=total_sec:
            ...
            if self.skipped==True:
                self.songProgress["value"]=50
                self.stopThread=True
                break
            if self.stopThread is True:
                break
            if self.isPaused==True:
                self.current_val=curr_sec
                self.stopThread=True
                break
        else:
            self.skip_scale.set(0)
    ...
    def play_song(self):
        ...
        try:
            ...
            self.my_player.set_skipped(self.skipped_val)
    ...
    def add_song(self):
        song_list=self.my_player.add_song()
    ...
    def skip_song(self,val):
        if self.isPlaying:
            skipped_value=float(self.skip_scale.get())
            self.skipped_val=(skipped_value*self.song_length)/100
            self.skipped=True
            self.songProgress['value']=int(self.skipped_val)
            self.current_val=int(self.skipped_val)
    ...
    def load_next_song(self):
        self.next_song_index=self.sel_song_index_tuple[0]
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple)==0:
                raise  NoSongSelectedError("Please select a song before adding it to the favourites!")
            ...
            messagebox.showerror("DBError","Song can not be added!")
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result.find("No songs present")!=-1:
                messagebox.showinfo("Favourites Empty!","No Songs Present In Your Favourites")
                return
            ...
            messagebox.showerror("DBError","Song can not be loaded!")
            print(traceback.format_exc())
